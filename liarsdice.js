...
//     };
   ...
   let names = ["Shirleen", "Kara", "Cleveland","Merri", "Conception", "Haley", "Florance", "Dorie", "Luella", "Vernia", "Freeman", "Katharina", "Charmain", "Graham", "Darnell", "Bernetta", "Inell", "Page", "Garnett", "Annalisa", "Brant", "Valda", "Viki", "Asuncion", "Moira", "Kaycee", "Richelle", "Elicia", "Eneida", "Evelynn"];
...
// ###############OBJECTS AT TABLE##############
    class Player {
        ...
        this.displayTurn = () =>{
            if (this.player){
                hideElements([bluffButton,spotOnButton, passButton, nextPlayerButton, nextRoundButton]);
                displayElements([inputs, declareButton]);
                playerDisplay.innerText = `${this.name}'s Turn`;
            }else{
                hideElements([inputs, declareButton, nextPlayerButton, nextRoundButton]);
                displayElements([bluffButton,spotOnButton, passButton]);
                playerDisplay.innerText = `${this.name}'s Turn`;
            }
        };
        // this.placeBet = () =>{
        //     if(this.player){
        //         displayElements([declareButton, inputs]);
        // }else{
        //         displayElements([bluffButton, passButton, spotOnButton]);
        //         console.log(`Player is betting`);
        //         let currentHandInts = countFaces(this);
        //         let largestCount = Math.max(...currentHandInts);
        //         let bestBet = [currentHandInts.indexOf(largestCount)+1, largestCount];
        //         if (aiCheckCurrentHandValidity(bestBet)){
        //             if (Math.random() > .8){
        //                 bestBet[1] += 1;
        ...
        //             while (aiCheckCurrentHandValidity(bestBet) !== true){
        //                 bestBet[1] += 1;
        //             }
        //             if (Math.random() > .8){
        //                 bestBet[1] += 1;
        ...
        //     }
            this.humanPlay = function{
                declareButton.addEventListener('click', () => {
                    ...
                    let bet = checkIfBetValid(face, count);
                    if(bet !== false){
                        //players bet is submitted
                    }else{
                        //ask for redo of inputs
                    }
                });
            };
            this.declareBet = function () {
                useDeclareButton(this);
            };
            this.aiBet = function(){
    ...
    const checkIfBetValid = (face, count, lastBet) => {
    ...
    const useDeclareButton = ()=>{declareButton.addEventListener('click', () => {
        let face = faceInput.value;
        let count = countInput.value;
        let bet = checkIfBetValid(face, count);
        if(bet !== false){
    ...
    let table = {
        ...
        challenged: null,
        getPlayers: function(){
            displayElements([nameInput, submit]);
            submit.addEventListener('click', ()=>{
                let name = nameInput.value;
                hideElements([nameInput]);
                let human = new Player(name, true);
                this.players.push(human);
                console.log(this.players[0]);
            });
            this.getAiPlayers();
        },
        getAiPlayers: function(call){
            displayElements([playersInput]);
            submit.addEventListener('click', ()=>{
                let num = playersInput.value;
                hideElements([playersInput, submit]);
                for (let i = 0; i< num; i++){
                    this.players.push(new Player());
                }
                this.initialSeatPlayers();
            })
        },

        initialSeatPlayers: function () {
            for(let i=0; i<this.players; i++){
                this.players[i].roll();
                this.players[i].addOccurrences();
            }
            console.log("Players are sat");
            this.startFirstPlayer();
        },

        startFirstPlayer: function () {
            this.currentPlayerObject = this.players[0];
            this.currentPlayerObject.displayTurn();
            this.playerPlays();
        },
        playerPlays: function(){
            this.currentPlayerObject.
        },
...
//Game Start Functions
    let cleanBoard = () => hideElements([bluffButton,spotOnButton,passButton,nextRoundButton,nextPlayerButton,faceDisplay,playerOptionsDisplay, declareButton, declareDisplay, inputs, result, submit, playersInput, nameInput]); //
    cleanBoard();
    let game = () => {
        table.getPlayers();
        // table.initialSeatPlayers();
        // table.startFirstPlayer();
    };
